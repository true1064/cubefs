...
func (eh *ExtentHandler) String() string {
	return fmt.Sprintf("ExtentHandler{ID(%v)Inode(%v)FileOffset(%v)Size(%v)StoreMode(%v)Status(%v)Dp(%v)storageClass(%v)}",
		eh.id, eh.inode, eh.fileOffset, eh.size, eh.storeMode, eh.status, eh.dp, eh.storageClass)
}
...
func (eh *ExtentHandler) appendExtentKey() (err error) {
	...
	if eh.key != nil {
		if eh.dirty {
			...
			status, err := eh.stream.client.appendExtentKey(eh.stream.parentInode, eh.inode, *eh.key, discard, eh.stream.isCache, eh.storageClass)
			...
			doAppend := func() (err error) {
				...
				status, err = eh.stream.client.appendExtentKey(eh.stream.parentInode, eh.inode, ekey, discard,
					eh.stream.isCache, proto.GetStorageClassByMediaType(eh.storageClass))
